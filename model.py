...
class R3Net(nn.Module):
    ...
    def forward(self, x):
        ...
        t=F.upsample(layer1, size=l0_size, mode='bilinear', align_corners=True)
        print(t.size())
        ...
        print('***********************reduce_low')
        print(reduce_low.shape)
        predict0 = self.predict0(reduce_high)
        print('***********************predict0')
        print(predict0.shape)
        print('***********************predict1')
        e1=torch.cat((predict0, reduce_low), 1)
        print(e1.shape)
        predict1 = self.predict1(e1) + predict0
